...
import { Bot, Settings, Code, Zap, MessageSquareMore, Key, ExternalLink, Send, MessageCircle, BookOpen, ShoppingCart, Coffee } from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, onOpenSettings, onOpenDonate }) => {
  return (
    <div className="w-full md:w-64 bg-slate-950 border-r border-slate-800 p-6 flex flex-col shrink-0">
      ...
      <nav className="flex-1 space-y-2">
         ...
         <button onClick={() => setActiveTab('config')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm font-medium transition-colors ${activeTab === 'config' ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/50' : 'text-slate-400 hover:bg-slate-800'}`}>
           <Settings size={18} /> Thiết kế EA của bạn
         </button>
        ...
        {onOpenSettings && (
          <div className="mb-4 space-y-2">
            <button 
              ...
              className="w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-xs font-bold text-slate-300 bg-slate-900 hover:bg-slate-800 border border-slate-800 hover:border-yellow-500/30 transition-all"
            >
            ...
            {onOpenDonate && (
              <button 
                ...
                className="w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-xs font-bold text-orange-400 bg-orange-500/10 hover:bg-orange-500/20 border border-orange-500/20 hover:border-orange-500/50 transition-all"
              >
